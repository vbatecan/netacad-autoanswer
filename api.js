...
async function getAiAnswersForBatch(questionsDataArray, apiKey) {
  ...
  prompt += "If a question implies multiple correct answers (e.g., 'select all that apply', 'choose N correct options'), include all correct answer texts for that question concatenated into a single string, separated by ' /// ' (space, three forward slashes, space). Example: 'Answer A /// Answer C'.\n";
  prompt += "Otherwise, if it's a single-choice question, return just the single best answer text as the string for that question.\n";
  prompt += "Return a single JSON array of strings, where each string is the processed answer for the corresponding question in the input array. Do not add any extra explanation or leading/trailing text.\n";
  prompt += "For example, if the input is two questions (Q1 single-choice, Q2 multi-choice requiring two answers), your output should be a JSON array like: [\"Text of answer for Q1\", \"Text of answer A for Q2 /// Text of answer B for Q2\"].\n\n";
